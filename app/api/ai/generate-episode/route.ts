...
【短剧节奏】单集全长约 1～3 分钟；**每个场景对应一个独立镜头（单次 AI 生成片段）**，单镜头时长 **仅允许 3～8 秒**（写如 "5s" 或 "5 seconds"），禁止 15 秒、30 秒、1 分钟等长镜头；需要更长叙事请增加 scenes 条数。建议约 12～20 个场景以撑满单集。
...
请生成剧本，包含：
1. 场景列表（条数符合短剧单集 1～3 分钟、每镜 3～8 秒）
      ...
      messages: [
        {
          role: 'system',
          content:
            '你是竖屏短剧编剧，擅长写**信息完整、可直接用于拍摄与 AI 视频**的场景段落。每个场景的 duration 只能是 3～8 秒的单镜头；单集约 1～3 分钟靠多条场景拼接。每个场景的 description 必须详尽成段，优先写满视觉与空间信息。',
        },
        {
          role: 'user',
          content: prompt,
